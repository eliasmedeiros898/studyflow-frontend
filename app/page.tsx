...
import {
  BarChart3, Bell, BookOpen, CalendarDays, Check, ChevronRight, CirclePlus,
...
type SubjectDetails = { subject: Subject; metrics: SubjectMetrics; topics: TopicSummary[]; recentSessions: StudySession[] };
...
export default function HomePage() {
  ...
  const toggleTask = async (id: string) => {
    ...
    try { await fetch(`${API_URL}/tasks/${id}/toggle`, { method: "PATCH" }); } catch { /* demo mode */ }
  };
...
function SubjectsView({ subjects, isDemo, setModal, onUpdate, onArchive, onRestore }: { subjects: Subject[]; isDemo: boolean; setModal: (value: "subject") => void; onUpdate: (subject: Subject) => void; onArchive: (subject: Subject) => void; onRestore: (subject: Subject) => void }) {
  ...
  const openDetails = async (subject: Subject) => {
    ...
    if (isDemo) {
      ...
      setDetails({ subject, metrics: metric, topics: metric.sessionCount ? [{ topic: "Conteúdo introdutório", sessionCount: metric.sessionCount, minutes: metric.minutes, questions: metric.questions, correctAnswers: metric.correctAnswers, accuracy: metric.accuracy, lastStudiedOn: today }] : [], recentSessions: metric.sessionCount ? [{ id: `demo-${subject.id}`, subjectId: subject.id, subjectName: subject.name, subjectColor: subject.color, topic: "Conteúdo introdutório", durationMinutes: 50, date: today, questions: 10, correctAnswers: 7, type: "FIRST_CONTACT", accuracy: 70, createdAt: new Date().toISOString() }] : [] }); setDetailsLoading(false); return;
    }
...
function ReviewsView({ isDemo, subjects, revision, onChanged }: { isDemo: boolean; subjects: Subject[]; revision: number; onChanged: () => void }) {
  ...
  return <div className="page reviews-page"><section className="page-title"><div><span className="eyebrow">MEMÓRIA EM DIA</span><h1>Central de revisões</h1><p>Acompanhe o que precisa voltar ao seu radar e mantenha o conteúdo vivo.</p></div></section>
    ...
    <section className="review-progress-section"><div className="panel-heading"><div><h2>Evolução por assunto</h2><p>O domínio considera seu resultado mais recente e o histórico acumulado.</p></div></div>{!loading && progress.length === 0 ? <div className="metric-empty"><BarChart3 /><strong>Responda questões para acompanhar sua evolução</strong></div> : <div className="review-progress-grid">{progress.map(item => <article className={`review-progress-card ${item.status.toLowerCase()}`} key={`${item.subjectId}-${item.topic}`}><button className="review-progress-main" onClick={() => setExpanded(current => current === `${item.subjectId}-${item.topic}` ? null : `${item.subjectId}-${item.topic}`)} aria-expanded={expanded === `${item.subjectId}-${item.topic}`}><span className="mastery-badge">{statusLabel(item.status)}</span><div className="review-progress-title"><i style={{ background: item.subjectColor }} /><span><strong>{item.topic}</strong><small>{item.subjectName}</small></span></div><div className="review-progress-metrics"><span><strong>{item.latestAccuracy}%</strong><small>último resultado</small></span><span><strong>{item.trend == null ? "—" : `${item.trend > 0 ? "+" : ""}${item.trend} p.p.`}</strong><small>tendência</small></span><span><strong>{item.answeredSessions}</strong><small>tentativas</small></span></div><div className="review-progress-footer"><small>{item.nextReviewDate ? `Próxima em ${formatShortDate(item.nextReviewDate)}` : "Sem revisão pendente"}</small><ChevronRight /></div></button>{expanded === `${item.subjectId}-${item.topic}` && <div className="review-history"><strong>Histórico de resultados</strong>{item.history.map(attempt => <div key={attempt.sessionId}><span>{formatShortDate(attempt.date)}</span><span>{attempt.correctAnswers}/{attempt.questions} questões</span><b>{attempt.accuracy}%</b></div>)}</div>}</article>)}</div>}</section>
...
function CompleteReviewModal({ task, isDemo, close, completed }: { task: Task; isDemo: boolean; close: () => void; completed: (task: Task, next: Task | null) => void }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/reviews/${task.id}/complete`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nextReviewDate }) });
  ...
  return <div className="modal-backdrop" onMouseDown={close}><section className="modal complete-review-modal" role="dialog" aria-modal="true" aria-labelledby="complete-review-title" onMouseDown={event => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">CONTINUIDADE</span><h2 id="complete-review-title">Concluir revisão</h2></div><button className="icon-button" onClick={close} aria-label="Fechar"><X /></button></div><p className="complete-review-topic">Você revisou <strong>{task.title.replace(/^Revisar:\s*/, "")}</strong>. Quando esse assunto deve voltar?</p><form onSubmit={submit}><div className="review-next-options">{[["1", "Amanhã"], ["3", "Em 3 dias"], ["7", "Em 7 dias"], ["15", "Em 15 dias"], ["none", "Não agendar"], ["custom", "Escolher data"]].map(([value, label]) => <label key={value} className={choice === value ? "selected" : ""}><input type="radio" name="nextReview" value={value} checked={choice === value} onChange={() => setChoice(value)} /><span>{label}</span></label>)}</div>{choice === "custom" && <label>Próxima revisão<input type="date" min={addDays(today, 1)} value={customDate} onChange={event => setCustomDate(event.target.value)} required /></label>}{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={submitting}>{submitting ? "Concluindo…" : "Concluir revisão"}</button></div></form></section></div>;
}
...
function SessionHistoryView({ isDemo, subjects, onChanged }: { isDemo: boolean; subjects: Subject[]; onChanged: () => void }) {
  ...
  return <div className="page history-page"><section className="page-title"><div><span className="eyebrow">SUA TRAJETÓRIA</span><h1>Histórico de estudos</h1><p>Consulte, corrija ou remova registros sem perder a visão do seu progresso.</p></div></section>
    ...
    <section className="panel history-filters"><label>Disciplina<select value={subjectId} onChange={event => setSubjectId(event.target.value)}><option value="">Todas</option>{subjects.map(subject => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label><label>De<input type="date" value={from} onChange={event => setFrom(event.target.value)} /></label><label>Até<input type="date" value={to} min={from || undefined} onChange={event => setTo(event.target.value)} /></label>{(subjectId || from || to) && <button className="secondary" onClick={() => { setSubjectId(""); setFrom(""); setTo(""); }}>Limpar filtros</button>}</section>
...
function EditSessionModal({ session, subjects, isDemo, close, saved }: { session: StudySession; subjects: Subject[]; isDemo: boolean; close: () => void; saved: (session: StudySession) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={close}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="edit-session-title" onMouseDown={event => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">CORRIGIR REGISTRO</span><h2 id="edit-session-title">Editar sessão</h2></div><button className="icon-button" onClick={close} aria-label="Fechar"><X /></button></div><form onSubmit={submit}><label>Disciplina<select name="subjectId" defaultValue={session.subjectId}>{subjects.map(subject => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label><label>Assunto estudado<input name="topic" defaultValue={session.topic} required maxLength={120} /></label><div className="form-row"><label>Tipo<select name="type" defaultValue={session.type}><option value="FIRST_CONTACT">Primeiro contato</option><option value="REVIEW">Revisão</option><option value="MOCK_EXAM">Simulado</option><option value="OTHER">Outro</option></select></label><label>Data<input name="date" type="date" defaultValue={session.date} required /></label></div><div className="form-row three"><label>Duração (min)<input name="durationMinutes" type="number" min="1" max="1440" defaultValue={session.durationMinutes} required /></label><label>Questões<input name="questions" type="number" min="0" defaultValue={session.questions} required /></label><label>Acertos<input name="correctAnswers" type="number" min="0" defaultValue={session.correctAnswers} required /></label></div><p className="review-rule-note"><Sparkles />Se o resultado mudar, a revisão automática pendente será recalculada.</p>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={submitting}>{submitting ? "Salvando…" : "Salvar alterações"}</button></div></form></section></div>;
}
...
function EditTaskModal({ task, subjects, isDemo, close, saved }: { task: Task; subjects: Subject[]; isDemo: boolean; close: () => void; saved: (task: Task) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={close}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="edit-task-title" onMouseDown={event => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">REORGANIZAR</span><h2 id="edit-task-title">Editar tarefa</h2></div><button className="icon-button" onClick={close} aria-label="Fechar"><X /></button></div><form onSubmit={submit}><label>Disciplina<select name="subjectId" defaultValue={task.subjectId} required>{subjects.filter(subject => !subject.archived || subject.id === task.subjectId).map(subject => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label><label>Título da tarefa<input name="title" defaultValue={task.title} required maxLength={120} autoFocus /></label><div className="form-row"><label>Data<input name="date" type="date" defaultValue={task.date} required /></label><label>Tipo<select name="type" defaultValue={normalizeTaskType(task.type)}><option value="FIRST_CONTACT">Primeiro contato</option><option value="REVIEW">Revisão</option><option value="EXAM">Prova ou simulado</option><option value="GENERAL">Geral</option></select></label></div>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary" onClick={close}>Cancelar</button><button className="primary" disabled={submitting}>{submitting ? "Salvando…" : "Salvar alterações"}</button></div></form></section></div>;
}
...
function Modal({ type, subjects, isDemo, focusMinutes, defaultTaskDate, close, addSubject, addTask, saveSession }: { type: "session" | "focusSession" | "task" | "subject"; subjects: Subject[]; isDemo: boolean; focusMinutes: number; defaultTaskDate: string; close: () => void; addSubject: (subject: Subject) => void; addTask: (task: Task) => void; saveSession: (minutes: number, questions: number, correct: number, scheduledReview?: ReviewSchedule | null) => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={close}><section className="modal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={event => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">NOVO REGISTRO</span><h2 id="modal-title">{titles[type]}</h2></div><button className="icon-button" onClick={close} aria-label="Fechar"><X /></button></div><form onSubmit={submit}>
    ...
    {type !== "subject" && <label>Disciplina<select name="subjectId" required>{subjects.filter(subject => !subject.archived).map(subject => <option value={subject.id} key={subject.id}>{subject.name}</option>)}</select></label>}
    {type === "task" && <><label>Título da tarefa<input name="title" required placeholder="O que você vai estudar?" autoFocus /></label><div className="form-row"><label>Data<input name="date" type="date" required defaultValue={defaultTaskDate} /></label><label>Tipo<select name="type"><option value="FIRST_CONTACT">Primeiro contato</option><option value="REVIEW">Revisão</option><option value="EXAM">Prova ou simulado</option><option value="GENERAL">Geral</option></select></label></div></>}
    {(type === "session" || type === "focusSession") && <><label>Assunto estudado<input name="topic" required placeholder="Ex.: Função de segundo grau" autoFocus /></label><div className="form-row"><label>Tipo de estudo<select name="sessionType" defaultValue="FIRST_CONTACT"><option value="FIRST_CONTACT">Primeiro contato</option><option value="REVIEW">Revisão</option><option value="MOCK_EXAM">Simulado</option><option value="OTHER">Outro</option></select></label><label>Data<input name="date" type="date" defaultValue={today} required /></label></div><div className="form-row three"><label>Duração (min)<input name="minutes" type="number" min="1" max="1440" required defaultValue={type === "focusSession" ? focusMinutes : 50} /></label><label>Questões<input name="questions" type="number" min="0" defaultValue="0" /></label><label>Acertos<input name="correct" type="number" min="0" defaultValue="0" /></label></div>{type === "focusSession" && <p className="focus-session-hint"><Clock3 />Tempo líquido recuperado do temporizador. Complete os dados para transformar o foco em progresso.</p>}<p className="review-rule-note"><Sparkles />Com questões respondidas, o assunto volta automaticamente: abaixo de 50% em 1 dia; 50–69% em 3; 70–84% em 7; 85% ou mais em 15 dias.</p></>}
