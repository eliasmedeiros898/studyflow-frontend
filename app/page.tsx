...
type TopicSummary = { topic: string; sessionCount: number; minutes: number; questions: number; correctAnswers: number; accuracy: number; lastStudiedOn: string; sessions?: StudySession[] };
...
function taskTypeToSessionType(type: string) {
  return ({ FIRST_CONTACT: "FIRST_CONTACT", REVIEW: "REVIEW", EXAM: "MOCK_EXAM", GENERAL: "OTHER" } as Record<string, string>)[normalizeTaskType(type)] ?? "OTHER";
}
...
function DashboardView({ firstName, timezone, dashboard, tasks, subjects, toggleTask, openTaskModal, openSession, startFocus, startTask, goTo }: { firstName: string; timezone: string; dashboard: Dashboard; tasks: Task[]; subjects: Subject[]; toggleTask: (id: string) => void; openTaskModal: () => void; openSession: () => void; startFocus: () => void; startTask: (task: Task) => void; goTo: (value: string) => void }) {
  const goalPercent = Math.min(100, Math.round(dashboard.minutesStudied / dashboard.weeklyGoalMinutes * 100));
  const completedTasks = tasks.filter(task => task.completed).length;
  const pendingTasks = tasks.filter(task => !task.completed);
  const orderedTasks = [...pendingTasks, ...tasks.filter(task => task.completed)];
  const taskProgress = tasks.length ? Math.round(completedTasks * 100 / tasks.length) : 0;
  return <div className="page dashboard-page">
    <section className="welcome">
      ...
      <button className="primary" onClick={startFocus}><Play size={18} fill="currentColor" />Iniciar foco</button>
    </section>
    ...
    <section className="dashboard-grid lower">
      <article className="panel tasks-panel">
        <div className="panel-heading"><div><h2>Plano de hoje</h2><p>{tasks.length ? pendingTasks.length ? `${pendingTasks.length} ${pendingTasks.length === 1 ? "tarefa pendente" : "tarefas pendentes"}` : "Tudo concluído por hoje" : "Nenhuma tarefa planejada"}</p></div><button className="text-button" onClick={() => goTo("Calendário")}>Abrir calendário <ChevronRight size={17} /></button></div>
        {tasks.length > 0 && <div className="today-progress" aria-label={`${taskProgress}% do plano concluído`}><i style={{ transform: `scaleX(${taskProgress / 100})` }} /></div>}
        {tasks.length ? <div className="task-list">{orderedTasks.map(task => <TaskRow key={task.id} task={task} subject={subjects.find(subject => subject.id === task.subjectId)} toggle={() => toggleTask(task.id)} onStart={!task.completed ? () => startTask(task) : undefined} />)}</div> : <div className="today-plan-empty"><CalendarDays /><div><strong>Seu dia ainda está livre</strong><p>Planeje um conteúdo para estudar ou comece uma sessão espontânea.</p></div></div>}
        <button className="add-line" onClick={openTaskModal}><CirclePlus size={18} />Adicionar ao calendário</button>
      </article>
      <article className="panel quick-panel">
        <div className="panel-heading"><div><h2>Ações rápidas</h2><p>Registre sem perder o ritmo</p></div></div>
        <button onClick={startFocus}><span className="quick-icon purple"><Play size={19} /></span><div><strong>Iniciar uma sessão</strong><small>Use o temporizador de foco</small></div><ChevronRight size={18} /></button>
        <button onClick={openSession}><span className="quick-icon green"><Plus size={19} /></span><div><strong>Registrar estudo</strong><small>Adicione uma sessão já feita</small></div><ChevronRight size={18} /></button>
        <button onClick={openTaskModal}><span className="quick-icon orange"><CalendarDays size={19} /></span><div><strong>Planejar tarefa</strong><small>Organize os próximos estudos</small></div><ChevronRight size={18} /></button>
      </article>
    </section>
  </div>;
}
...
function TaskRow({ task, subject, toggle, onEdit, onDelete, onStart }: { task: Task; subject?: Subject; toggle: () => void; onEdit?: () => void; onDelete?: () => void; onStart?: () => void }) {
  return <div className={`task-row ${task.completed ? "completed" : ""}`}><button className="check-button" onClick={toggle} aria-label={task.completed ? "Reabrir tarefa" : "Concluir tarefa"}>{task.completed && <Check size={15} />}</button><span className="subject-line" style={{ background: subject?.color ?? task.subjectColor }} /><div><strong>{task.title}</strong><small>{subject?.name ?? task.subjectName ?? "Disciplina"} · {taskTypeLabel(task.type)}{task.origin === "AUTOMATIC_REVIEW" && <em>Automática</em>}</small></div>{onStart && <button className="task-start" onClick={onStart}>{task.origin === "AUTOMATIC_REVIEW" ? <RotateCcw /> : <Play fill="currentColor" />}{task.origin === "AUTOMATIC_REVIEW" ? "Revisar" : "Estudar"}</button>}{onEdit || onDelete ? <span className="task-actions">{onEdit && <button onClick={onEdit} aria-label={`Editar ${task.title}`}><Pencil /></button>}{onDelete && <button onClick={onDelete} aria-label={`Excluir ${task.title}`}><Trash2 /></button>}</span> : null}</div>;
}
...
function SubjectDetailsModal({ details, loading, close }: { details: SubjectDetails | null; loading: boolean; close: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={close}><section className="modal subject-detail-modal" role="dialog" aria-modal="true" aria-labelledby="subject-detail-title" onMouseDown={event => event.stopPropagation()}><div className="modal-heading"><div><span className="eyebrow">VISÃO DA DISCIPLINA</span><h2 id="subject-detail-title">{details.subject.name}</h2></div><button className="icon-button" onClick={close} aria-label="Fechar"><X /></button></div><section className="subject-detail-summary"><article><Clock3 /><span><small>Tempo total</small><strong>{formatMinutes(details.metrics.minutes)}</strong></span></article><article><BookOpen /><span><small>Sessões</small><strong>{details.metrics.sessionCount}</strong></span></article><article><Target /><span><small>Acurácia</small><strong>{details.metrics.questions ? `${details.metrics.accuracy}%` : "—"}</strong></span></article></section><section className="subject-topic-section"><div className="panel-heading"><div><h2>Assuntos estudados</h2><p>Abra um assunto para consultar cada sessão registrada.</p></div></div>{details.topics.length ? <div className="subject-topic-list">{details.topics.map(topic => { const key = topic.topic.trim().toLocaleLowerCase("pt-BR"); const expanded = expandedTopic === key; const topicSessions = topic.sessions ?? details.recentSessions.filter(session => session.topic.trim().toLocaleLowerCase("pt-BR") === key); return <article className={expanded ? "expanded" : ""} key={key}><button className="subject-topic-trigger" onClick={() => setExpandedTopic(current => current === key ? null : key)} aria-expanded={expanded}><i style={{ background: details.subject.color }} /><span className="subject-topic-name"><strong>{topic.topic}</strong><small>{topic.sessionCount} {topic.sessionCount === 1 ? "sessão" : "sessões"}</small></span><span className="subject-topic-metric"><small>Tempo total</small><strong>{formatMinutes(topic.minutes)}</strong></span><span className="subject-topic-metric"><small>Acurácia geral</small><strong>{topic.questions ? `${topic.accuracy}%` : "Sem questões"}</strong></span><ChevronDown /></button>{expanded && <div className="subject-topic-sessions"><div className="subject-topic-sessions-head"><span>Data e tipo</span><span>Tempo</span><span>Resultado</span></div>{topicSessions.map(session => <div className="subject-topic-session" key={session.id}><span><time dateTime={session.date}>{formatShortDate(session.date)}</time><small>{sessionTypeLabel(session.type)}</small></span><strong>{formatMinutes(session.durationMinutes)}</strong><span className={session.questions ? "has-result" : "no-result"}>{session.questions ? <><strong>{session.accuracy}%</strong><small>{session.correctAnswers}/{session.questions} questões</small></> : <small>Sem questões</small>}</span></div>)}</div>}</article>; })}</div> : <div className="compact-empty">Nenhum assunto registrado.</div>}</section><section className="subject-review-section"><div className="panel-heading"><div><h2>Revisões da disciplina</h2><p>Próximos retornos e revisões já concluídas</p></div><span className="subtle-badge">{reviews.filter(review => !review.completed).length} pendentes</span></div>{reviews.length ? <div className="subject-review-list">{reviews.map(review => <article key={review.id}><i style={{ background: details.subject.color }} /><div><strong>{review.title.replace(/^Revisar:\s*/, "")}</strong><small>Revisão automática</small></div><span className={`subject-review-status ${review.completed ? "done" : review.date < today ? "late" : "pending"}`}>{reviewStatus(review)}</span><time dateTime={review.date}>{formatShortDate(review.date)}</time></article>)}</div> : <div className="compact-empty">Nenhuma revisão agendada para esta disciplina.</div>}</section></section></div>;
}
...
function Modal({ type, subjects, isDemo, focusMinutes, defaultTaskDate, sessionTask, close, addSubject, addTask, saveSession }: { type: "session" | "focusSession" | "task" | "subject"; subjects: Subject[]; isDemo: boolean; focusMinutes: number; defaultTaskDate: string; sessionTask: Task | null; close: () => void; addSubject: (subject: Subject) => void; addTask: (task: Task) => void; saveSession: (minutes: number, questions: number, correct: number, scheduledReview?: ReviewSchedule | null) => void }) {
  const [error, setError] = useState("");
  const submit = async (event: FormEvent<HTMLFormElement>) => {
